import { describe, expect, it } from 'vitest'
import { AGENDA_MAX_ENTRIES, buildCalendarAgenda } from './calendar-agenda'
import type { CalendarEvent } from './calendar-types'
import type { Automation } from './automations-types'

const HOUR = 60 * 60 * 1000
const DAY = 24 * HOUR
// Local midnight, not Date.UTC: the RRULE engine resolves BYHOUR/BYMINUTE with
// local-time Date methods, so a UTC-anchored base makes these assertions
// timezone-dependent and flaky off UTC.
const BASE = new Date(2026, 0, 5, 0, 0, 0, 0).getTime() // Monday, local time

function event(overrides: Partial<CalendarEvent> = {}): CalendarEvent {
  return {
    id: 'evt-1',
    title: 'Dentist',
    startAt: BASE + 9 * HOUR,
    endAt: BASE + 10 * HOUR,
    allDay: false,
    notes: null,
    source: 'local',
    createdAt: BASE,
    updatedAt: BASE,
    ...overrides
  }
}

function automation(overrides: Partial<Automation> = {}): Automation {
  return {
    id: 'auto-1',
    name: 'Nightly PR digest',
    prompt: 'summarize',
    precheck: null,
    agentId: 'claude',
    projectId: 'repo-1',
    executionTargetType: 'local',
    executionTargetId: 'local',
    schedulerOwner: 'local_host_service',
    workspaceMode: 'existing',
    workspaceId: 'ws-1',
    baseBranch: null,
    reuseSession: false,
    timezone: 'UTC',
    rrule: 'FREQ=DAILY;BYHOUR=8;BYMINUTE=0',
    dtstart: BASE,
    enabled: true,
    nextRunAt: BASE + 8 * HOUR,
    missedRunPolicy: 'run_once_within_grace',
    missedRunGraceMinutes: 720,
    createdAt: BASE,
    updatedAt: BASE,
    ...overrides
  } as Automation
}

describe('buildCalendarAgenda', () => {
  it('returns only events when there are no automations', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [event()],
      automations: [],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda).toHaveLength(1)
    expect(agenda[0]).toMatchObject({ kind: 'event', startAt: BASE + 9 * HOUR })
  })

  it('returns only automation runs when there are no events', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [],
      automations: [automation()],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda).toHaveLength(1)
    expect(agenda[0]).toMatchObject({
      kind: 'automation-run',
      automationId: 'auto-1',
      name: 'Nightly PR digest'
    })
  })

  it('returns an empty agenda when both sources are empty', () => {
    expect(
      buildCalendarAgenda({ events: [], automations: [], from: BASE, to: BASE + DAY })
    ).toEqual({ entries: [], truncated: false })
  })

  it('merges both sources sorted by start time', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [event()],
      automations: [automation()],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda.map((entry) => entry.kind)).toEqual(['automation-run', 'event'])
    expect(agenda[0].startAt).toBeLessThan(agenda[1].startAt)
  })

  it('expands a recurring automation across multiple days', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [],
      automations: [automation()],
      from: BASE,
      to: BASE + 3 * DAY
    })
    expect(agenda).toHaveLength(3)
  })

  it('excludes events entirely outside the window', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [event({ id: 'past', startAt: BASE - 5 * DAY, endAt: BASE - 5 * DAY + HOUR })],
      automations: [],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda).toEqual([])
  })

  it('includes an event that overlaps the window boundary', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [event({ id: 'straddle', startAt: BASE - HOUR, endAt: BASE + HOUR })],
      automations: [],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda).toHaveLength(1)
    expect(agenda[0]).toMatchObject({ kind: 'event' })
  })

  it('skips disabled automations', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [],
      automations: [automation({ enabled: false })],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda).toEqual([])
  })

  it('returns an empty agenda when the window is inverted', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [event()],
      automations: [automation()],
      from: BASE + DAY,
      to: BASE
    })
    expect(agenda).toEqual([])
  })

  it('reports truncated: false for an agenda that fits', () => {
    expect(
      buildCalendarAgenda({ events: [event()], automations: [], from: BASE, to: BASE + DAY })
        .truncated
    ).toBe(false)
  })

  it('truncates at AGENDA_MAX_ENTRIES so a wide window cannot explode', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [],
      automations: [automation({ rrule: 'FREQ=HOURLY;BYMINUTE=0' })],
      from: BASE,
      to: BASE + 365 * DAY
    })
    expect(agenda).toHaveLength(AGENDA_MAX_ENTRIES)
  })

  it('flags an agenda that hit the entry ceiling', () => {
    const agenda = buildCalendarAgenda({
      events: [],
      automations: [automation({ rrule: 'FREQ=HOURLY;BYMINUTE=0' })],
      from: BASE,
      to: BASE + 365 * DAY
    })
    expect(agenda.truncated).toBe(true)
  })

  // The reported failure: automation expansion consumed the shared budget first,
  // so late user events sorted past index 500 and disappeared from every consumer.
  it('never evicts a user event to make room for automation runs', () => {
    const hourly = (id: string): Automation =>
      automation({ id, name: id, rrule: 'FREQ=HOURLY;BYMINUTE=0' })
    const events = [163, 164, 165, 166, 167].map((hour) =>
      event({
        id: `sunday-${hour}`,
        title: `Dinner ${hour}`,
        startAt: BASE + hour * HOUR,
        endAt: BASE + hour * HOUR + HOUR
      })
    )
    const agenda = buildCalendarAgenda({
      events,
      automations: [hourly('auto-a'), hourly('auto-b'), hourly('auto-c')],
      from: BASE,
      to: BASE + 7 * DAY
    })
    expect(agenda.entries.filter((entry) => entry.kind === 'event')).toHaveLength(events.length)
    expect(agenda.entries.length).toBeLessThanOrEqual(AGENDA_MAX_ENTRIES)
    expect(agenda.truncated).toBe(true)
  })

  it('keeps every user event when events alone overflow the ceiling', () => {
    const events = Array.from({ length: AGENDA_MAX_ENTRIES + 20 }, (_unused, index) =>
      event({
        id: `evt-${index}`,
        startAt: BASE + index * 60_000,
        endAt: BASE + index * 60_000 + HOUR
      })
    )
    const agenda = buildCalendarAgenda({
      events,
      automations: [automation()],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda.entries).toHaveLength(AGENDA_MAX_ENTRIES)
    expect(agenda.entries.every((entry) => entry.kind === 'event')).toBe(true)
    expect(agenda.truncated).toBe(true)
  })

  it('skips an automation whose schedule cannot be expanded', () => {
    const { entries: agenda } = buildCalendarAgenda({
      events: [event()],
      automations: [automation({ rrule: 'not-a-real-rule' })],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda).toHaveLength(1)
    expect(agenda[0]).toMatchObject({ kind: 'event' })
  })
})

function googleEvent(overrides: Partial<CalendarEvent> = {}): CalendarEvent {
  return { ...event(), id: 'google:cal:1', source: 'google', title: 'From Google', ...overrides }
}

describe('buildCalendarAgenda — external events', () => {
  it('merges external events alongside local ones, sorted by start', () => {
    const agenda = buildCalendarAgenda({
      events: [event({ startAt: BASE + 10 * HOUR, endAt: BASE + 11 * HOUR })],
      externalEvents: [googleEvent({ startAt: BASE + 9 * HOUR, endAt: BASE + 10 * HOUR })],
      automations: [],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda.entries).toHaveLength(2)
    expect(agenda.entries[0].startAt).toBeLessThan(agenda.entries[1].startAt)
  })

  it('omitting externalEvents behaves exactly as before', () => {
    const agenda = buildCalendarAgenda({
      events: [event()],
      automations: [],
      from: BASE,
      to: BASE + DAY
    })
    expect(agenda.entries).toHaveLength(1)
  })

  it("never evicts the user's own events in favour of external ones", () => {
    const externals = Array.from({ length: AGENDA_MAX_ENTRIES + 50 }, (_unused, index) =>
      googleEvent({
        id: `google:cal:${index}`,
        startAt: BASE + index * 60_000,
        endAt: BASE + index * 60_000 + 30_000
      })
    )
    const mine = event({ id: 'mine', startAt: BASE + 20 * HOUR, endAt: BASE + 21 * HOUR })
    const agenda = buildCalendarAgenda({
      events: [mine],
      externalEvents: externals,
      automations: [],
      from: BASE,
      to: BASE + DAY
    })
    expect(
      agenda.entries.some((entry) => entry.kind === 'event' && entry.event.id === 'mine')
    ).toBe(true)
    expect(agenda.truncated).toBe(true)
  })
})
